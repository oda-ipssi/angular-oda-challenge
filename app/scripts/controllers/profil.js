  ...
  .controller('ProfilCtrl', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.updateProfil = function() {
    	...
    	if (typeof $scope.profil.username === 'undefined' || !$scope.profil.username.length) {
      	...
      	else if($scope.profil.password && $scope.profil.password.length){

      		if (typeof $scope.profil.passwordConfirm === 'undefined' || !$scope.profil.passwordConfirm.length) {
		        $scope.errorMessage = 'Veuillez entrer un mot de passe de confirmation';
		        check = false;
		    } else if (!angular.equals($scope.profil.password, $scope.profil.passwordConfirm)) {
		        $scope.errorMessage = 'Veuillez confirmer votre mot de passe';
		        check = false;
		    } else {
		    	data.data.password = $scope.profil.password;
		    }
      	} 
    ...
    $scope.$on('$viewContentLoaded', function(event) {
      ...
      $http.get('http://127.0.0.1:6969/userProfil.json').then(
        function(response) {
